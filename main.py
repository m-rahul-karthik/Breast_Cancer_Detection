...
for training_name in train_labels:
    ...
    for fileName in os.listdir(dir):
        ...
        ###################################
        global_feature = np.hstack([fv_haralick,fv_LBP])
...
#print(sum_features1)
print("mfs",sum(global_feature)/len(global_feature))
print("stdev",statistics.stdev(global_feature)/len(global_feature))
...
for name, model in models:
    ...
    print(prediction)
    fpr, tpr, thresholds = metrics.roc_curve(testLabelsGlobal, prediction)
    k=metrics.auc(fpr, tpr)
    tn, fp, fn, tp=metrics.confusion_matrix(testLabelsGlobal, prediction).ravel()
    ...
    if(1==1):
        ...
        plt.figure(1)
        plt.plot([0, 1], [0, 1], 'k--')
        line= plt.plot(fpr_keras, tpr_keras, label=name + ' ({:.3f}, p<0.0001)'.format(auc_keras),linewidth=2.0)
